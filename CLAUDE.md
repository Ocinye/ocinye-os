...
- **Ocinye Core: `IMPLEMENTED` e em produção.** 201 caminhos e 239 operações
...
- **Ocinye Workspace: `IMPLEMENTED` e em produção** 89 ecrãs em Leptos SSR,
...
- **Ocinye Mail: `IMPLEMENTED`, `NOT CONFIGURED`.** Módulo do Core com
  ...
  **recusa injecção de cabeçalho** à porta: uma quebra de linha ou carácter de
  ...
  O compositor tem **anexos**: os bytes passam pela fronteira de armazenamento
  pessoal — quota admitida, tipo validado, chave opaca, dono (§40, ADR-0108) —,
  ficam ligados ao rascunho (`mail_draft_attachments`), viajam no envio como
  `multipart/mixed`, e são libertados ao descartar o rascunho, sem objectos
  órfãos. Limites de tamanho, total e número são impostos no servidor.
- **Governança de recursos: `IMPLEMENTED` (fundação), sem enforcement.** O
  ...
  novas param. O membro **vê** a sua quota: o ecrã «Meus Recursos» mostra uso,
...
- **Ficheiros pessoais: `IMPLEMENTED`, um explorador de conteúdo, em produção**
  ...
  **Quick Look**: imagem e PDF inline, texto e código escapados, e uma ficha com
  descarregar para o resto. As **miniaturas** aparecem na grelha, geradas por um
  trabalhador: de imagem (descodificada no processo), da primeira página de um
  PDF, de um documento de **Office** (Word, Excel, PowerPoint, OpenDocument, RTF —
  o LibreOffice converte-o a PDF primeiro) e de um fotograma de **vídeo** (com o
  `ffmpeg`). Toda a conversão de conteúdo não confiável — poppler, LibreOffice,
  ffmpeg — corre num **contentor descartável e endurecido**, criado pelo
  **Conversion Runner**, o único serviço com o socket do Docker; o worker nunca
  corre parsers hostis nem contentores no próprio processo, e comprometê-lo não dá
  root sobre o host ([ADR-0609](docs/adrs/0609-disposable-conversion-isolation.md)).
  Há **selecção múltipla** com mover/eliminar em lote, **arrastar para mover**, e
  ...
  **descarga institucional**, que redireccionava para uma ligação assinada
  ...
  ([ADR-0608](docs/adrs/0608-same-origin-institutional-downloads.md)).
...
- **1647 funções de teste** escritas na árvore, e **zero falhas** na última
  ...
  **610 dessas funções não correm sem base de dados** — vivem em ficheiros que leem
