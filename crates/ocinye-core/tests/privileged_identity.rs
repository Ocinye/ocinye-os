...
async fn a_pessoa_do_bootstrap_ganha_acesso_sem_duplicar() {
    ...
    assert_eq!(
        antes, 1,
        "havia mais do que um Fidel Monteiro antes de provisionar"
    );
    ...
    let agora = identity::person_by_id(&pool, dono)
        .await
        .expect("l")
        .expect("e");
    let principal = identity::principal_for_person(&pool, &agora)
        .await
        .expect("principal");
    assert!(
        !principal.roles.contains(&TechnicalRole::PlatformAdmin),
    ...
    assert!(
        !principal.identity_kind.is_privileged(),
        "a pessoa tornou-se privilegiada"
    );
}
...
async fn uma_identidade_privilegiada_nao_se_provisiona() {
    ...
    sqlx::query(
        "UPDATE credentials SET state = 'revoked', revoked_at = now() WHERE person_id = $1",
    )
    .bind(privilegiada.id)
    .execute(&pool)
    .await
...
async fn a_auditoria_resolve_a_pessoa_por_tras_da_identidade() {
    ...
    .expect("registo");
    assert_eq!(
        actor_id, privilegiada.id,
        "o registo não diz qual identidade executou"
    );
    assert_eq!(
        accao, "account_provisioned",
        "a acção não distingue provisionar de criar"
    );
...
async fn nao_se_provisiona_alguem_de_outra_organizacao() {
    ...
    let (estrangeira, _) = identity::bootstrap_privileged_identity(
        ...
        alheia,
        HumanOwner {
            full_name: "Alguém de Outra Casa".to_owned(),
            email: outro_humano,
        },
        "Outra Admin",
        &outro_admin,
