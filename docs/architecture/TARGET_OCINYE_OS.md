...
| **Perfis** | **`CURRENT` desde a Parte 2** ([ADR-0014](../adrs/0014-instance-profiles-and-application-activation.md)): quatro perfis, activação de aplicações por Instância, essenciais não desactiváveis, unidades só em `research`, Ficheiros relevante a todo o membro interno | terminologia configurável por perfil («departamento» em vez de «unidade»); papéis ainda com nomes de investigação | 2 ✓ |
| **Fronteira Core/aplicações** | **`CURRENT` desde a Parte 3** ([ADR-0015](../adrs/0015-core-and-applications-boundary.md), [fronteira](CORE_AND_APPLICATIONS.md)): o Core recusa a API de uma aplicação inactiva (`503 application_inactive`) e mantém identidade, saúde e configuração; um `panic` de aplicação é um `500` e não derruba o Core | isolamento de processo, rotas declaradas pela aplicação (Parte 4) | 3 ✓ |
| **Manifesto de aplicação** | **`CURRENT` desde a Parte 4** ([ADR-0016](../adrs/0016-application-manifest-contract.md)): manifesto nos contratos, partilhado; rotas da API declaradas; pedidos de armazenamento, rede, IA e recursos | aplicações não nativas, manifestos fora do binário | 4 ✓ |
| **Nós** | **`CURRENT` desde a Parte 5** ([ADR-0504](../adrs/0504-node-capacity-model.md), [docs/nodes](../nodes/README.md)): capacidade física/reservada/alocável/alocada/consumida, reserva do operador, capacidade da Instância, ciclo A–H provado | identidade criptográfica do nó (pares de chaves, mTLS) → Parte 12; despacho de trabalhos | 5 ✓ |
| **Segredos** | **`CURRENT` desde a Parte 6** ([ADR-0110](../adrs/0110-instance-secrets-authority.md)): armazém selado por domínio HKDF, nunca devolvido, uso por âmbito dentro do Core, rotação, revogação que apaga, auditoria sem valor | rotação da própria raiz de selagem | 6 ✓ |
