...
| **Ficheiros institucionais** — `File`, versões, pastas | implementado; descarga servida bytes same-origin (ADR-0608) | ecrã Ficheiros: navegação, pastas, largar, carregar, detalhes, histórico, descarga | MinIO local; nenhum armazenamento institucional | `AVAILABLE` em desenvolvimento |
| **Ficheiros pessoais** — explorador de conteúdo (ADR-0607) | implementado; serve bytes same-origin (sem endpoint público) | grelha/lista, Quick Look (imagem/PDF/texto), miniaturas (imagem + 1ª página de PDF), selecção em lote, arrastar para mover, Favoritos, Recentes, Lixo | MinIO local; PDF rasterizado num contentor descartável e endurecido pelo Conversion Runner (ADR-0609) | `AVAILABLE` em produção — adiado: miniaturas de Office/vídeo (a fronteira de conversão já as recebe; falta o conversor pesado) |
