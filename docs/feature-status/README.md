...
| **Ficheiros pessoais** — explorador de conteúdo (ADR-0607) | implementado; serve bytes same-origin (sem endpoint público) | grelha/lista, Quick Look (imagem/PDF/texto), miniaturas (imagem, PDF, Office e vídeo), selecção em lote, arrastar para mover, Favoritos, Recentes, Lixo | MinIO local; poppler/LibreOffice/ffmpeg num contentor descartável e endurecido pelo Conversion Runner (ADR-0609) | `AVAILABLE` em produção |
