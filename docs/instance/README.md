...
- A marca de uma Instância: parte seguinte do programa ([arquitectura-alvo](../architecture/TARGET_OCINYE_OS.md)).
...
Uma Instância tem um **perfil** — `research`, `business`, `education` ou
`personal` — que decide as predefinições: que aplicações opcionais começam
activas, e se nasce com a estrutura inicial de unidades (só `research`). **Não
concede autoridade**, muda-se depois da instalação em Administração › Instância,
e mudar não apaga nada ([ADR-0014](../adrs/0014-instance-profiles-and-application-activation.md)).

**Criar uma Instância exige escolher o perfil** — `--profile` no `bootstrap-admin`,
ou `OCINYE_INSTANCE_PROFILE` —: não há perfil por omissão. A primeira Instância,
a da Ocinye, é `research`.
...
```bash
ocinye-core-server bootstrap-admin --instance-name "Cooperativa Exemplo" --profile business …
```
...
| Cada perfil nasce com as suas aplicações e a sua estrutura; activar, desactivar e repor | `crates/ocinye-core/tests/instance_profiles.rs` |
| Desactivar esconde sem tocar no Core, reactivar devolve os dados | `apps/workspace/tests/browser.rs`, `desactivar_uma_aplicacao_esconde_a_e_reactivar_devolve_a_intacta` |
