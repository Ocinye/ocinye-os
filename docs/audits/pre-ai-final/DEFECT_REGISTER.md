...
| F-22 | P1 | i18n | **Ecrãs de idioma misto em produção.** Com o francês activo, a navegação vinha em francês mas o corpo dos ecrãs (Home, e os restantes módulos) continuava em português — porque só a navegação fora migrada para o catálogo; os corpos ainda tinham literais fixos. **Não** era fragmentação de idioma: a barra e o corpo renderizam no mesmo pedido, sob o mesmo idioma; o corpo saía em português por não passar por `t()`. Dois pontos genuinamente arquitecturais: o feed de Actividade («Idea created:») e os badges de estado/classificação («INTERNAL») vêm de dados, não do catálogo. Encontrado na aceitação autenticada em produção (Home em francês com corpo português). | **EM CURSO (fatia 2)** — Home migrado por inteiro (saudação, subtítulo com plurais, KPIs, cartões, painel de IA, acesso rápido, estados vazios); `classification_badge` e `task_state_badge` passam a traduzir o rótulo mantendo o enum; guarda de **pureza de idioma** (`o_home_nao_mistura_linguas`, com prova por marcas) impede o chrome misto de voltar. Prosa vinda do Core (resumos de actividade, mensagem de IA) marcada como conteúdo, a localizar em fatia própria do Core. Os restantes ~30 ecrãs (lists, administração, correio, ficheiros, calendário, …) seguem em fatias próprias — ~2 900 strings ao todo. |
